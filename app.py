...
def notlari_yukle():
    ...
    query = "SELECT * FROM notlar WHERE 1=1"
    params = []

    if arama:
        query += " AND LOWER(baslik) LIKE LOWER(?)"
        params.append('%' + arama + '%')
    if secilen_kategori != "Tüm Kategoriler":
        query += " ORDER BY CASE WHEN LOWER(kategori) = LOWER(?) THEN 0 ELSE 1 END, tarih DESC"
    ...
    if not notlar:
        bos_label = customtkinter.CTkLabel(
            liste_frame,
    ...
    for i, not_ in enumerate(notlar):
        ...
        baslik_label = customtkinter.CTkLabel(
            sol_frame, text=f"📌 {baslik}", font=("Segoe UI", 16, "bold"),
        ...
        alt_label = customtkinter.CTkLabel(
            sol_frame, text=alt_bilgi, font=("Segoe UI", 11),
            text_color=text_color, anchor="w"
        )
        ...
        def duzenle_fonksiyonu(not_id=id, baslik_=baslik, kategori_=kategori, icerik_=icerik, onem_=onem):
            yeni_not_penceresi(guncelle=True, not_id=not_id, baslik=baslik_, kategori=kategori_, icerik=icerik_,
                               onem=onem_)

        def sil_fonksiyonu(not_id=id):
            conn_inner = sqlite3.connect("not_defteri.db")
            cursorr = conn_inner.cursor()
            cursorr.execute("DELETE FROM notlar WHERE id = ?", (not_id,))
            conn_inner.commit()
            conn_inner.close()
            notlari_yukle()

        duzenle_buton = customtkinter.CTkButton(
            sag_frame, text="✏️", width=35, height=35, corner_radius=10,
            fg_color="#3498db", hover_color="#2980b9", command=duzenle_fonksiyonu
        )
        duzenle_buton.pack(side="top", pady=2)

        sil_buton = customtkinter.CTkButton(
            sag_frame, text="🗑️", width=35, height=35, corner_radius=10,
            fg_color="#e74c3c", hover_color="#c0392b", command=sil_fonksiyonu
        )
...
def yeni_not_penceresi(guncelle=False, not_id=None, baslik="", kategori="Genel", icerik="", onem="Orta"):
    ...
    text_widget = tk.Text(text_container, wrap="word", font=("Segoe UI", 11), bg="#ffffff", fg="#2c3e50", relief="flat", bd=10, insertbackground="#3498db", selectbackground="#3498db")
...
arama_buton = customtkinter.CTkButton(
    arama_frame, text="Ara", width=60, command=notlari_yukle
)
arama_buton.pack(side="left", padx=(5,0))
